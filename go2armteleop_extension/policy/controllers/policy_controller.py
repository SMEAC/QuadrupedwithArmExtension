...
from ..core.single_articulation import SingleArticulation
...
class PolicyControllerGo2(BaseController):
    """
    ...
    def initialize(
        ...
        leg_joint_indices = np.arange(12, dtype=np.int64)
        if set_gains:
            self.robot._articulation_view.set_gains(stiffness, damping, joint_indices = leg_joint_indices)
        if set_limits:
            self.robot._articulation_view.set_max_efforts(max_effort, joint_indices = leg_joint_indices)
